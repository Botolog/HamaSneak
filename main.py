...
@app.route("/UR/<act>/<size>")  # type: ignore
def update_rout(act, size):
    with open("rout.tes", "a") as rout:
        rout.write(f"{act}{size}\n")

    if act.upper() == "F":
        return f"The car went Forward by {size}"
    if act.upper() == "B":
        return f"The car went Backward by {size}"
    if act.upper() == "R":
        return f"The car turned Right by {size}"
    if act.upper() == "L":
        return f"The car turned Left by {size}"
    if act.upper() == "S":
        return f"The Scanned the aria"
