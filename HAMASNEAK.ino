...
// void driveIR(int speed, float hardness = 1.5, int duration = 0)
// {
//   duration = duration * 1000;
//   unsigned long now = millis();
//   while (now + duration > millis())
...
//     drive(speed, 0, (IRLval - IRRval) / hardness);
//     delay(20);
...
void uploadScan(int arrnum, int arr[180]){
  String str = "https://GALER-TES.botologe.repl.co/US/"+String(arrnum)+"/";
  for (int i=0; i<179; i++){
    str += String(arr[i]) +",";
  } 
  str += String(arr[179]);
...
void uploadRout(char act, int Size){
  String str = "https://GALER-TES.botologe.repl.co/UR/"+String(act)+"/"+String(Size);
  Serial.println(str);
  http.begin(str);
  int httpCode = http.GET();                                                 
    if (httpCode > 0) { //Check for the returning code
